...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        ...
        # ファイルのアップロード
        netcat.py -t 127.0.0.1 -p 5555 -l -u=mytest.txt
        # コマンドの実行
        netcat.py -t 127.0.0.1 -p 5555 -l -e=\"cat /etc/passwd\"
        # 通信先サーバーの135番ポートに文字列を送信
        echo 'ABC | .netcat.py -t 127.0.0.1 -p 135
        # サーバーに接続
    ...
    parser.add_argument('-e', '--execute', help='対話型シェルの初期化')

    parser.add_argument('-l', '--listen', action='store_true', help='通信待受モード')

    parser.add_argument('-p', '--port', type=int, default='127.0.0.1')
