...
HEX_FILTER = ''.join(
    [(len(repr(chr(i))) == 3) and chr(i) or '.' for i in range(256)]        # 印字可能：len=3、印字不可能：len=!3
)

#　パケットの詳細を16進数の値とASCIIで印字可能な文字の両方で出力する関数
...
def proxy_handler(client_socket, remote_host, remote_port, recieve_first):
    ...
    while True:
        ...
        if not len(local_buffer) or not response_handler(remote_buffer):
            client_socket.close()
            remote_socket.close()
